...
class User extends Service
{
    ...
    protected $allowFields = ['id', 'username', 'nickname', 'avatar', 'email', 'mobile','vip_type'];
    ...
    public function mobileCodeRegLogin($params){
        try{
            ...
            if(!$user){
                $str = $params['lngX'].','.$params['latY'];
                $data = [
                    'mobile' => $params['mobile'],
                    'login_time' => date('Y-m-d H:i:s'),
                    'login_ip'   => request()->ip(),
                    'mobile_area' => get_mobile_area($params['mobile']),
                    'reg_area'=> getAddress($params['lngX'].','.$params['latY']),
                    'nickname'=>GetRandStr(6),
                    'avatar'=>'/static/index/image/service.jpg'
                ];
                $user = \app\common\model\User::create($data, true);
                $this->_user = \app\common\model\User::get($user->id);
            }else{
