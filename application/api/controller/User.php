...
({
    ...
    "data": {
        ...
        "username": "",
        "nickname": "",
        "avatar": "http://www.layTp.com/static/index/image/default.png",
...
({
    ...
    public function mobile_code_reg_login()
    {
        ...
        $param['lngX'] = $this->request->param('lngX');
        $param['latY'] = $this->request->param('latY');

        $validate = new MobileCodeRegLogin();
        if($validate->check($param)){
            if($this->service_user->mobileCodeRegLogin($param)){
                $this->success('登录成功', ['token'=>$this->service_user->getToken(),'userInfo'=>$this->service_user->getUserInfo()]);
            }else{
